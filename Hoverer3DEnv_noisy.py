...
class Hoverer3DEnv(gym.Env):
    def __init__(self):
        ...
        self.pos_th = 0.5 # Position Threshold: must stay within a 1x1x1 meter box
    ...
    def step(self, action):
        ...
        thrust_ang += np.random.normal(0, math.pi/180/50, size=thrust_ang.shape) # apply noise
        ...
        pos += np.random.normal(0, 0.001, size=pos.shape)
        vel += np.random.normal(0, 0.005, size=vel.shape)
        ang += np.random.normal(0, math.pi/180/20, size=ang.shape)
        angvel += np.random.normal(0, math.pi/180/10, size=angvel.shape)
